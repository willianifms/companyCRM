...
import { 
  LayoutDashboard, Calendar, Target, Compass, Percent, Database, Network, Kanban, ShieldCheck, Cpu
} from 'lucide-react';
    ...
    <aside className="w-full md:w-64 bg-slate-900/50 border-r border-slate-900 p-4 flex flex-col gap-5 shrink-0 overflow-y-auto">
      <div className="space-y-1">
        ...
        <button 
          ...
          className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all ${
            activeTab === 'qualification' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:bg-slate-900 hover:text-white'
          }`}
        >
          <Cpu size={16} />
          <span>Qualificação de IA</span>
        </button>
      </div>
